...
import { APPROVED_STUDY_QUESTIONS } from "../src/platform/ending-study.ts";
...
const PATH_BEATS = Object.freeze({
  "man-to-pool": ["b05"],
  "pool-wash-to-return": ["b05"],
  "pool-to-neighbors": ["b06"],
  "group-to-inquiry": ["b08"],
  "parents-entry": ["b11"],
  "parents-exit": ["b12"],
  "parents-entry-exit": ["b11", "b12"],
  expulsion: ["b17"],
  "jesus-entry": ["b18"],
  ending: ["b19"],
});
...
async function buildPlan() {
  ...
  const observerProhibitionsEntry = {
    ...
    sourceLevel: "S3",
    content: "禁止玩家控制耶穌、造成醫治、代替經文人物回答、重排事件、改寫見證、選擇神學結論或改變結局。",
  ...
  original.scripture.artifact.reviewerRegistryEvidence.sha256 =
    ownerReviewSha256;
  original.scripture.verses = original.scripture.verses.map((verse) => ({
    ...verse,
  ...
  original.recallGoalSheet = {
    ...
    recalls: RECALL_INTERACTIONS.map((recall, index) => ({
      id: recall.id,
      beatId: recall.afterBeatId,
      prompt: APPROVED_STUDY_QUESTIONS[index].prompt,
      ledgerEntryIds: [ledgerId("S2", recall.afterBeatId)],
      answerBasisVerseKeys: [
        ...APPROVED_STUDY_QUESTIONS[index].verseKeys,
      ],
      onIncorrect: "continue-with-neutral-feedback",
  ...
  original.completionContract = {
    condition: "Complete all nineteen ordered John 9 beats without changing scripture cause, testimony, order, theology, or outcome.",
    requiredBeatIds: STORY_BEATS.map(({ id }) => id),
    completionStateId: STORY_COMPLETION.id,
    policyRuleIds: [],
    restartBehavior: "Clear committed progress, replay b01-b19 in canonical order, and converge on the same immutable b19 final state.",
    reentryBehavior: "Restore only a validated canonical beat prefix, resume at the next beat, and converge on the same immutable b19 final state.",
  };

  const acceptanceUpdates = {
    "accept-event-spine": ["passed", "Nineteen ordered event rows bind runtime beats b01-b19 to exact verse keys and S1 ledger entries."],
    "accept-player-role": ["passed", "The canonical warrant and runtime enforce the unnamed observer's non-causal action limits."],
    "accept-beats": ["passed", "Nineteen canonical Beat contracts match runtime order, triggers, goals, sequences, dialogue and final states."],
    ...
    "accept-recall-goals": ["passed", "Nineteen non-punitive goals and three optional scoreless recalls are bound to runtime evidence."],
    "accept-graybox-world": ["passed", "The canonical plan binds the tested 2560x1792 five-region world, routes, portals, collisions, camera and responsive targets."],
    "accept-sequence-finality": ["passed", "All nineteen sequences have identical normal, skip, restart and re-entry parity hashes."],
    ...
    "accept-completion": ["passed", `Completion requires b01-b19 and the immutable ${STORY_COMPLETION.id} contract ending at ${STORY_COMPLETION.finalSnapshotId}.`],
  };
  const acceptanceCategories = {
    "accept-event-spine": "Nineteen ordered scripture beats",
    "accept-beats": "Nineteen implementation-bound Beats",
    "accept-completion": "All nineteen Beats complete",
  };
...
if (process.argv.includes("--check")) {
  ...
  console.log("Canonical 19-beat story plan is synchronized.");
} else {
  await writeFile(PLAN_PATH, generated);
  console.log("Synchronized canonical 19-beat story plan.");
}
