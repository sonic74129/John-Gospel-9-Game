...
export function createAppShell(
  ...
  const qaFixtureMarkup = import.meta.env.DEV
    ...
    : "";
  const qaReviewWarningMarkup =
    '<p class="review-warning">個人使用候選版：經文已綁定固定來源；等待擁有者完成單次逐節審核。</p>';
  const qaLicenseNoticeMarkup =
    '<span class="license-notice">和合本（繁體神版）· Public Domain 來源</span>';
  ...
  const presentLines = (
    ...
    const renderLine = (): void => {
      ...
      if (import.meta.env.DEV) {
        ...
        dialoguePlaceholder.textContent = line.exactText;
        dialogueMetadata.replaceChildren(
          ...
          metadataRow("Text SHA-256", line.textSha256),
        );
      } else {
        dialogueSource.textContent = line.sourceLabel;
        dialoguePlaceholder.textContent = line.exactText;
        dialogueMetadata.replaceChildren();
      }
